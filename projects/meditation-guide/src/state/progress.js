...
export const initialProgress = {
  ...
  totalSeconds: 0,
  notes: {},
  preferences: { duration: 180, eyesOpen: false, reducedMotion: false },
};
...
export function completeSession(state, { day, duration, note }) {
  ...
  return {
    ...state,
    completed: first ? [...state.completed, day] : state.completed,
    totalSeconds: state.totalSeconds + (first ? duration : 0),
    notes: note ? { ...state.notes, [day]: note } : state.notes,
  };
}
export function recommendNext(state) {
  return state.completed.length
    ? Math.min(7, Math.max(...state.completed) + 1)
    : 1;
}
